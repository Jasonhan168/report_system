...
import {
  ...
  initDefaultData, checkUserReportPermission, getReportModuleDatasource,
} from "./db";
import { generateMockWipData, generateMockFilterOptions } from "./mockData";
import { getClickHouseClient, testClickHouseConnection, invalidateClickHouseClient } from "./datasource";
import { queryWipData, queryWipFilterOptions, exportWipData } from "./queries/pkg-wip-summary";
import {
  queryOutsourceOrderDetail,
  queryOutsourceOrderFilterOptions,
  exportOutsourceOrderDetail,
} from "./queries/outsource-order-detail";
import {
  queryWipDetail,
  ...
  pkgWipSummary: router({
    ...
    filterOptions: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        // 动态路由：根据数据源类型决定使用 ClickHouse 还是 Mock
        const ds = await getReportModuleDatasource("pkg_wip_summary");
        if (ds && ds.type === "clickhouse") {
          const client = getClickHouseClient(ds);
          return queryWipFilterOptions(client, input.date);
        }
        return generateMockFilterOptions(input.date);
      }),
    ...
    query: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (ds && ds.type === "clickhouse") {
          const client = getClickHouseClient(ds);
          return queryWipData(client, input);
        }
        return generateMockWipData(input);
      }),
    ...
    exportData: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (ds && ds.type === "clickhouse") {
          const client = getClickHouseClient(ds);
          return exportWipData(client, input);
        }
        return generateMockWipData({ ...input, page: 1, pageSize: 99999 });
      }),
  }),

  // ─── 委外订单明细表 ─────────────────────────────────────────────────────────
  outsourceOrderDetail: router({
    checkPermission: protectedProcedure
      .input(z.object({ type: z.enum(["view", "export"]) }))
      .query(async ({ ctx, input }) => {
        if (ctx.user.role === "admin") return { allowed: true };
    ...
    filterOptions: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (ds && ds.type === "clickhouse") {
          const client = getClickHouseClient(ds);
          return queryOutsourceOrderFilterOptions(client, input.date);
        }
        return { productionTypes: [], vendorNames: [], plants: [] };
      }),
    ...
    query: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (ds && ds.type === "clickhouse") {
          const client = getClickHouseClient(ds);
          return queryOutsourceOrderDetail(client, input);
        }
        return { rows: [], total: 0 };
      }),
    ...
    exportData: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (ds && ds.type === "clickhouse") {
          const client = getClickHouseClient(ds);
          return exportOutsourceOrderDetail(client, input);
        }
        return [];
      }),
  }),
  ...
  pkgWipDetail: router({
    ...
    filterOptions: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (ds && ds.type === "clickhouse") {
          const client = getClickHouseClient(ds);
          return queryWipDetailFilterOptions(client, input.date);
        }
        return { vendorNames: [] };
      }),
    ...
    query: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (ds && ds.type === "clickhouse") {
          const client = getClickHouseClient(ds);
          return queryWipDetail(client, input);
        }
        return { rows: [], total: 0 };
      }),
