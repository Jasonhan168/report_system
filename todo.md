...
## 2026-04-19 第五批优化
- [x] 修复返回汇总表跳转路径（/reports/pkg-wip-summary）
- [x] 去掉明细表筛选栏中“筛选条件已锁定…”提示文字
- [x] 委外订单明细表新增“预计交期”列（edd字段，位于下单日期后）
- [x] 委外订单明细表新增“拖期天数”列（最后一列，当前日期-预计交期>0则显示天数否则不显示）
- [x] 委外订单明细表交期预警高亮：拖期行红色高亮，临近交期（<=5天）黄色高亮

## 2026-04-21 第六批优化
- [x] WIP汇总表跳转委外订单明细表：仅显示该行关联的订单号（后端 order_nos 过滤，前端不显示）
- [x] 返回汇总表时保持原有过滤条件（URL 携带 summaryDate/summaryLabelName/summaryVendorName）
