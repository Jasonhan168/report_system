...
export default function OutsourceOrderDetail() {
  ...
  const { data: filterOptions } = trpc.outsourceOrderDetail.filterOptions.useQuery(
    { date: date || undefined },
    { enabled: !!permission?.allowed }
  );
  const { data, isLoading, refetch } = trpc.outsourceOrderDetail.query.useQuery(
    queryParams,
    { enabled: !!permission?.allowed }
  );
  ...
  async function handleExport() {
    ...
    try {
      ...
      const parts = ["委外订单明细表", date, vendorName, labelName].filter(Boolean);
  ...
  return (
    <div className="flex flex-col h-full gap-3 p-4">
      ...
      <div className={`flex flex-wrap gap-2 items-end bg-card border rounded-lg p-3 ${filterLocked ? "ring-1 ring-blue-300 bg-blue-50/30" : ""}`}>
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            ...
            <SelectContent>
              ...
              {filterOptions?.productionTypes?.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="__all__">全部</SelectItem>
              {filterOptions?.plants?.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span>共 <strong className="text-foreground">{data?.total ?? 0}</strong> 条记录</span>
        <Badge variant="outline" className="text-amber-500 border-amber-500">回货率 &lt; 98%</Badge>
      </div>
        ...
        <Table>
          <TableHeader className="sticky top-0 bg-muted/80 backdrop-blur z-10">
            <TableRow>
              ...
              <TableHead className="whitespace-nowrap">ERP料号</TableHead>
              ...
              <TableHead className="whitespace-nowrap text-right">下单数量</TableHead>
