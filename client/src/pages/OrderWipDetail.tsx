...
import {
  ...
  AlertCircle, X, ChevronsUpDown, Filter,
} from "lucide-react";
...
export default function OrderWipDetail() {
  ...
  // 统计表/汇总表跳转过来的标识 + 返回路由 + 要恢复的筛选
  const fromPivot = sp.get("fromPivot") === "1";
  const fromSummary = sp.get("fromSummary") === "1";
  const fromOther = fromPivot || fromSummary;
  const backRoute = sp.get("backRoute") || (fromSummary ? "/reports/pkg-wip-inproc-summary" : "/reports/pkg-unissued-pivot");
  ...
  // 从统计表/汇总表跳转时默认冻结筛选条件
  const [filterLocked, setFilterLocked] = useState(fromOther);
  ...
  const { data: viewPerm }   = trpc.orderWipDetail.checkPermission.useQuery({ type: "view" });
  const { data: exportPerm } = trpc.orderWipDetail.checkPermission.useQuery({ type: "export" });
  const { data: moduleMeta } = trpc.reportModules.getByCode.useQuery({ code: "order_wip_detail" });

  const { data: filterOpts } = trpc.orderWipDetail.filterOptions.useQuery(
    undefined,
    { enabled: !!viewPerm?.allowed },
  );

  const { data, isLoading, isFetching, error, refetch } = trpc.orderWipDetail.query.useQuery(
    queryParams,
    { enabled: !!viewPerm?.allowed },
  );
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        ...(queryParams.plant          ? { plant:          queryParams.plant }          : {}),
      });
      const resp = await fetch(`/api/export/order-wip-detail?${params.toString()}`);
      if (!resp.ok) {
      ...
      const nameParts = ["封装在制品明细表", today];
      ...
      else if (queryParams.vendorPartNo)   nameParts.push(queryParams.vendorPartNo);
      else if (queryParams.productionType) nameParts.push(queryParams.productionType);
      else if (queryParams.plant)          nameParts.push(queryParams.plant);
    ...
    <div ref={tableRef} className="flex flex-col h-full p-4 gap-4 report-page">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-xl font-bold text-foreground">封装在制品明细表</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <FuzzyCombobox
              ...
              value={vendorName}
              onChange={filterLocked ? () => {} : setVendorName}
          ...
          <div className="space-y-1.5">
            ...
            <FuzzyCombobox
              ...
              value={label}
              onChange={filterLocked ? () => {} : setLabel}
              placeholder="全部（可输入筛选）"
              emptyText="无匹配的标签品名"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <FuzzyCombobox
              ...
              value={vendorPartNo}
              onChange={filterLocked ? () => {} : setVendorPartNo}
              placeholder="全部（可输入筛选）"
              emptyText="无匹配的供应商料号"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <FuzzyCombobox
              ...
              onChange={filterLocked ? () => {} : setProductionType}
              placeholder="全部（可输入筛选）"
              emptyText="无匹配的类型"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <FuzzyCombobox
              ...
              value={plant}
              onChange={filterLocked ? () => {} : setPlant}
              placeholder="全部（可输入筛选）"
              emptyText="无匹配的分公司"
            />
          </div>
                  ...
                  {data?.data.map((row, idx) => {
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-right text-xs p-2 align-middle whitespace-nowrap">
                        ...
                        ) : row.overdue_days >= -5 && row.overdue_days <= 0 ? (
